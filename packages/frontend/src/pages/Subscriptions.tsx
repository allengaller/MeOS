...
function StatCard({ label, value, accent, accentColor = 'text-slate-900' }: StatCardProps) {
  return (
    <div className="relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-slate-50 to-white" />
      <div className="relative p-6 pt-5">
        <p className="text-xs font-medium text-slate-400 uppercase tracking-widest mb-2" style={{ fontSize: '9px', letterSpacing: '0.1em' }}>
          {label}
        </p>
        <p className={`text-3xl font-light ${accentColor} tracking-tight`}>
          {value}
        </p>
      </div>
    </div>
...
function QuotaModal({ subscriptionId, quota, onSave, onClose }: {
...
export default function Subscriptions() {
  const { dashboard, loading, fetchDashboard, createSubscription, updateSubscription, deleteSubscription, addQuota } = useSubscriptionStore();
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-end mb-2">
          <div>
            <h1 className="text-2xl font-light text-slate-900 tracking-tight">订阅管理</h1>
            <p className="text-sm text-slate-400 mt-1">追踪你的 IDE、Cloud、AI 订阅，最大化价值</p>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {dashboard?.subscriptions.length === 0 && (
          <div className="col-span-2 bg-white rounded-xl border border-dashed border-slate-200 p-12 text-center">
            ...
            <p className="text-sm text-slate-500 mb-1">还没有订阅</p>
            <p className="text-xs text-slate-400 mb-4">添加你的第一个订阅开始管理</p>
            <button
              onClick={() => setShowSubModal(true)}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm text-slate-600 hover:text-slate-900 border border-slate-200 hover:border-slate-300 rounded-lg transition-all"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
        ...
        {dashboard?.subscriptions.map((sub, index) => (
          <div key={sub.id} style={{ animationDelay: `${index * 80}ms` }}>
            <SubscriptionCard
              sub={sub}
              onEdit={() => { setEditingSub(sub as any); setShowSubModal(true); }}
              onDelete={() => handleDelete(sub.id)}
              onAddQuota={() => setEditingQuota({ subId: sub.id })}
            />
          </div>
        ))}
      </div>
